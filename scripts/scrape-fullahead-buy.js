...
async function scrapeWithRestart({ launchBrowserFn = launchBrowser } = {}) {
...
async function main({
  ...
  loadCardNumbersFn = loadCardNumbers,
  scrapeFn = scrapeWithRestart,
  readPreviousCountFn = readPreviousCount,
  nowFn = () => new Date(),
} = {}) {
  console.log('[fullahead-buy] Starting...');
  const startTime = Date.now();

  const cardNumbers = loadCardNumbersFn();
  ...
  const prevCount = readPreviousCountFn(outputFile);
