...
FONT_PATH_MAC_BOLD = "/System/Library/Fonts/Helvetica.ttc"
FONT_PATH_APPLE_SD = "/System/Library/Fonts/AppleSDGothicNeo.ttc"


def get_font(size: int, bold: bool = False) -> ImageFont.FreeTypeFont:
    for path in [FONT_PATH_APPLE_SD, FONT_PATH_MAC_BOLD, FONT_PATH_MAC]:
...
def generate_icon():
    """1024x1024 app icon — rounded-square, card-themed, 'HH' central mark."""
...
def generate_adaptive_icon():
    """1024x1024 adaptive foreground — transparent bg, safe-zone padded content."""
    img = Image.new("RGBA", (SIZE, SIZE), (0, 0, 0, 0))
    draw = ImageDraw.Draw(img)

    safe_margin = int(SIZE * 0.1667)
    safe_size = SIZE - 2 * safe_margin
    safe_x0, safe_y0 = safe_margin, safe_margin
...
def generate_splash():
    """1024x1024 splash logo — transparent bg, centered 'HoloHunter' mark."""
    img = Image.new("RGBA", (SIZE, SIZE), (0, 0, 0, 0))
    draw = ImageDraw.Draw(img)
...
def main():
    ...
    print("\nDone. CR note:")
    print("- icon.png: card-themed rounded icon, 'HH' mark, HoloHunter label")
    print("- adaptive-icon.png: circle within 66.7% safe zone, transparent bg for icon mask")
    print("- splash-icon.png: centered logo on transparent bg, used with white splash background")
