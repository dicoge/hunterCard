...
- [ ] 隱私權政策 URL：`https://holocard-hunter.vercel.app/privacy`（部署自 `public/privacy.html`，經 `vercel.json` rewrite；已外部驗證回 200）。
- [ ] Support URL / Marketing URL（可用同一個專案頁或網站）：`https://holocard-hunter.vercel.app/support`（部署自 `public/support.html`；已外部驗證回 200）。
